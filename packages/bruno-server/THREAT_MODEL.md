...
| `renderer:list-directory`/`create-directory`/`rename-directory` รับ opaque file handle (Improvement.md P1.1) แทน raw path ได้ — handle เป็น encrypted blob ไม่ตรงรูปแบบ absolute-path ที่ generic scanner (`findPathsInValue`) จับได้ ถ้าไม่แก้อะไรเพิ่มจะหลุดผ่าน `BRUNO_SERVER_ALLOWED_ROOTS` sandbox ไปฟรีๆ | `CHANNEL_PATH_EXTRACTORS` entry เฉพาะ 3 channel นี้ decode handle กลับเป็น real path ก่อนเช็ค `checkPathPolicy()` เสมอ — handle ที่ decode ไม่ได้ตกไปเช็คเป็น string เดิม (fail-safe) | `security/allowed-roots.js`, `bruno-electron`'s `utils/file-handles.js` |
