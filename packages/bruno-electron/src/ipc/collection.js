...
const registerRendererEventHandlers = (mainWindow, watcher) => {
  ...
  ipcMain.handle(
    ...
    async (event, collectionName, collectionFolderName, collectionLocation, options = {}) => {
      try {
        ...
        // Return the opened collection too, so Browser Mode can hydrate from
        // the IPC response if a WebSocket event is missed during reconnect.
        return {
          pathname: dirPath,
          uid,
          brunoConfig: persistedConfig
        };
      } catch (error) {
  ...
  ipcMain.handle('renderer:load-gql-schema-file', async (_, selectedPath = null) => {
    try {
  ...
  ipcMain.handle('renderer:export-collection-zip', async (event, collectionPath, collectionName, destinationPath = null) => {
    try {
