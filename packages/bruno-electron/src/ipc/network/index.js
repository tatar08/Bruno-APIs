...
const registerNetworkIpc = (mainWindow) => {
    ...
    'renderer:run-collection-folder', async function runCollectionFolderHandler(event, folder, collection, environment, runtimeVariables, recursive, delay, tags, selectedRequestUids, datasetRows, datasetInfo, runnerOptions = {}) {
      ...
      if (!runnerOptions.parallelChild) {
        mainWindow.webContents.send('main:run-folder-event', {
          ...
          datasetRows: datasetInfo ? iterations : null
        });
      }
  ...
  ipcMain.handle('renderer:save-response-to-file', async (event, response, url, pathname, destinationPath = null) => {
    try {
      ...
      const filePath = destinationPath || await chooseFileToSave(mainWindow, path.join(dirPath, fileName));
      if (filePath) {
