...
const InlineCollectionCreator = ({ onComplete, onCancel, onOpenAdvanced }) => {
  ...
  useEffect(() => {
    ...
    if (defaultLocation) {
      transport.invoke('renderer:find-unique-folder-name', 'Untitled Collection', defaultLocation)
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      ...
      // Inline creation does not expose a destination-folder field. Resolve a
      // safe folder name so existing data is never overwritten.
      const uniqueFolderName = await transport.invoke('renderer:find-unique-folder-name', folderName, defaultLocation);
      const targetFolderName = sanitizeName(uniqueFolderName);

      await dispatch(createCollection(name, targetFolderName, defaultLocation, { format: DEFAULT_COLLECTION_FORMAT }));
      toast.success(targetFolderName === folderName
        ? 'Collection created!'
        : 'Collection created in "' + targetFolderName + '"');
      onComplete();
    } catch (e) {
