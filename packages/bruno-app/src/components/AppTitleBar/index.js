...
const AppTitleBar = () => {
  ...
  const showWindowControls = isElectron && (isWindows || isLinux);

  // Listen for fullscreen changes
  useEffect(() => {
    transport.invoke('renderer:window-is-fullscreen')
      .then((fullscreen) => {
    ...
    const removeEnterFullScreenListener = transport.on('main:enter-full-screen', () => {
      setIsFullScreen(true);
    });

    const removeLeaveFullScreenListener = transport.on('main:leave-full-screen', () => {
  ...
  useEffect(() => {
    ...
    transport.invoke('renderer:window-is-maximized')
      .then((maximized) => {
    ...
    const removeMaximizedListener = transport.on('main:window-maximized', () => {
      setIsMaximized(true);
    });

    const removeUnmaximizedListener = transport.on('main:window-unmaximized', () => {
  ...
  const handleMinimize = useCallback(() => {
    transport.send('renderer:window-minimize');
  }, []);

  const handleMaximize = useCallback(() => {
    transport.send('renderer:window-maximize');
    // State will be updated via IPC events from main process (main:window-maximized/main:window-unmaximized)
  }, []);

  const handleClose = useCallback(() => {
    transport.send('renderer:window-close');
  }, []);
