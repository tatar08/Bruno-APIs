...
export default function Main() {
  ...
  useEffect(() => {
    // Browser Mode does not have Electron's BrowserWindow did-finish-load
    // lifecycle. The React tree is ready once this effect runs, so reveal it
    // immediately and let workspace hydration continue in the background.
    // This also supports older bridge processes without main:app-loaded.
    if (!transport.isElectron) {
      mainSectionRef.current?.setAttribute('data-app-state', 'loaded');
    }

    const removeAppLoadedListener = transport.on('main:app-loaded', (init = {}) => {
      if (mainSectionRef.current) {
        mainSectionRef.current.setAttribute('data-app-state', 'loaded');
      }
      setShowRosettaBanner(Boolean(init.isRunningInRosetta));
    });
