...
**ยังไม่ทำ (เหลือใน P0.4)**: session-scoped active workspace/active collection state, จำกัดจำนวน sessions/terminals/watchers ต่อ user — ทั้งหมดนี้ยังต้องมีแนวคิด "session ownership" แบบเดียวกับที่ terminal/watcher/cookie ใช้ แต่ผูกกับ resource type อื่น ซึ่งบางส่วน (เช่น จำกัดจำนวนต่อ user) เป็นการตัดสินใจ product เรื่อง limit ที่เหมาะสม (secret/credential ต่อ session ตรวจแล้วไม่ใช่ gap — ดู increment ที่หก)
