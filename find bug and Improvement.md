...
### P0.4 Per-Session Isolation — เฉพาะส่วน event routing (เสร็จแล้ว, ทำงานเมื่อเปิด auth P0.1 เท่านั้น)
...
**ยังไม่ทำ (เหลือใน P0.4)**: session-scoped active workspace/active collection state, terminal process isolation ต่อ session (ตอนนี้ terminal session ID เป็น namespace ของตัวเอง แต่ event/output ก่อนหน้านี้ก็ยังโดน global broadcast เหมือน channel อื่นซึ่งตอนนี้แก้แล้วในระดับ event, แต่ terminal process/PTY เองยังไม่ผูกกับ session สำหรับ access-control — ใครก็ตามที่รู้ terminal session ID (จาก HTTP call เดิม) ยังคุยกับ terminal นั้นได้ผ่าน channel เดิม ไม่ใช่แค่เจ้าของ), secret/credential isolation ต่อ session, reference-counted filesystem watcher cleanup ตอน session ปิด, จำกัดจำนวน sessions/terminals/watchers ต่อ user — ทั้งหมดนี้ต้องมีแนวคิด "session ownership" ที่ผูกกับ resource ฝั่ง backend จริง ๆ (ไม่ใช่แค่ event routing) ซึ่งเป็นงานใหญ่กว่านี้มาก และบางส่วน (เช่น จำกัดจำนวนต่อ user) เป็นการตัดสินใจ product เรื่อง limit ที่เหมาะสม
