...
**ยังไม่ทำ (เหลือใน P0.4)**: session-scoped active workspace/active collection state, secret/credential isolation ต่อ session, reference-counted filesystem watcher cleanup ตอน session ปิด, จำกัดจำนวน sessions/terminals/watchers ต่อ user, cleanup terminal ของ session อัตโนมัติตอน logout/session หมดอายุ (ตอนนี้ `DELETE /api/auth/session` ยังไม่ได้ผูกกับ `terminal-ownership.js`/`handlerRegistry` เพื่อ kill terminal ที่ค้างอยู่ — ต้องส่ง `handlerRegistry`/`windowShim`/`createFakeEvent` เข้าไปใน `routes/auth.js` ซึ่งตอนนี้ยังไม่ได้รับ dependency พวกนี้เลย เพิ่ม scope ตรงนี้เข้าไปจะทำให้ increment นี้ใหญ่ขึ้นและข้าม concern เดิมของไฟล์ auth routes) — ทั้งหมดนี้ยังต้องมีแนวคิด "session ownership" แบบเดียวกับที่ terminal ใช้ แต่ผูกกับ resource type อื่น ซึ่งบางส่วน (เช่น จำกัดจำนวนต่อ user) เป็นการตัดสินใจ product เรื่อง limit ที่เหมาะสม
