...
### P1.1 Server File Explorer and Transfer Center 🟡 เสร็จบางส่วน (folder-picking modal + upload/download + progress/cancel + create-folder/rename + file-picker multi-select/preview + search/recent/favorites + Bridge-machine label + checksum verification + opaque file handle API (backend) + resume สำหรับ download จริงแล้ว; resume สำหรับ upload ยังไม่ทำ (deferred แยกต่างหาก), opaque handle ฝั่ง frontend ยังไม่ทำ)
...
- ✅ **resume สำหรับ download ที่ค้างกลางทาง** (ขอบเขต download เท่านั้น — upload-resume defer แยกต่างหาก ดูด้านล่าง) — export handler ทั้งสองตัวหลัง `/api/downloads/:channel` (`renderer:export-collection-zip`, `renderer:export-workspace`) regenerate zip ใหม่ทุกครั้งที่เรียก ไม่การันตี byte-identical ระหว่าง 2 ครั้ง (archive timestamp/entry ordering ต่างกันได้) แปลว่าจะ retry ทั้ง request ใหม่พร้อม `Range` header ตรงๆ ไม่ปลอดภัย — client อาจได้ byte จากไฟล์คนละ generation มาต่อกัน checksum เดิมจับไม่ได้เพราะ cover แค่ response เดียว ไม่ใช่ไฟล์ที่ splice จาก 2 generation; แก้ด้วย resume-token registry ใหม่ `security/download-resume.js` — request แรกที่ generate export สำเร็จ เก็บ `tempPath` ที่เขียนไว้แล้วลง in-memory Map (`resumeToken -> { tempPath, sessionKey, sha256, downloadName, expiresAt }`, pattern เดียวกับ `auth.js`'s `sessions` และ `idempotency.js`'s `cache` — lazy TTL expiry ไม่มี sweep timer, bounded ด้วย `MAX_ENTRIES` evict ตัวเก่าสุดเมื่อเต็ม, TTL/max-entries override ได้ผ่าน `BRUNO_SERVER_DOWNLOAD_RESUME_TTL_MS`/`BRUNO_SERVER_DOWNLOAD_RESUME_MAX_ENTRIES`) แล้วส่ง `resumeToken` กลับใน header `X-Resume-Token`; ถ้า stream ขาดกลางทาง client retry ด้วย `resumeToken` เดิม — route (`routes/downloads.js`) เห็น `resumeToken` ใน body แล้ว **ข้ามการเรียก handler ใหม่ทั้งหมด** ไปเสิร์ฟไฟล์เดิมที่ tempPath เดิม ให้ Express's `res.download()` (ผ่าน `send` package ข้างใต้) จัดการ `Range`/206 ให้เองตามมาตรฐาน HTTP — ไม่ต้องทำ byte-resume protocol เองเลย แค่ต้อง "ชี้กลับไปไฟล์เดิมให้แม่นเท่านั้น"
  - **session-scoped**: `getResumeEntry(resumeToken, sessionKey)` เช็ค `req.brunoSessionId || req.ip` ของ request ที่ resume ต้องตรงกับ sessionKey ตอน token ถูกสร้างเป๊ะ กัน client อื่น (เดา/ขโมย token) ดึงไฟล์ export ของ client อื่นไปได้
  - **token lifecycle**: download ที่จบสมบูรณ์ (callback ของ `res.download()` ไม่มี error) discard token + ลบ tempPath ทันทีเพราะ resume ต่อไม่ได้อีกแล้ว (ส่งครบแล้ว); download ที่ error กลางทาง (client หลุดการเชื่อมต่อ) **ตั้งใจไม่ discard** ปล่อย token/ไฟล์ไว้ให้ resume จริงสำเร็จได้ก่อน TTL หมด (SCRATCH_DIR sweep เดิมเก็บกวาดไฟล์ที่ไม่มีใครมา resume อยู่แล้วภายใน 1 ชม.เป็น fallback)
  - **แก้ concurrency-slot timing inconsistency ระหว่างทำ**: branch resume เดิม release concurrency slot ข้างใน `res.download()`'s callback (รอ transfer ทั้งก้อนจบก่อนถึงปล่อย slot) ต่างจาก branch fresh-generation ที่ release ทันทีหลังเรียก `res.download()` (ไม่รอ) — inconsistent และ branch resume จะ hold slot นานเกินจำเป็นสำหรับไฟล์ใหญ่ แก้ให้ทั้งสอง branch release ทันทีหลัง kick off stream เหมือนกัน (ไม่ใช่รอใน callback)
  - **frontend**: `ipc-transport.js`'s `downloadWithProgress()` เดิม chunk-read fail แล้ว throw ตรงๆ — เพิ่ม bounded resume-retry (`MAX_DOWNLOAD_RESUME_ATTEMPTS = 3`): stream-read error ที่ไม่ใช่ cancellation (`signal?.aborted`/`AbortError` ยัง throw `TransferCancelledError` ทันทีเหมือนเดิม ไม่เปลี่ยน) และมี `resumeToken` จาก response header (`x-resume-token`) พร้อม attempt เหลือ → เรียก method ใหม่ `_resumeDownloadStream()` (fetch ซ้ำไป endpoint เดิมพร้อม `resumeToken` + `Range: bytes=${received}-` header, คาด 206 เท่านั้น) ได้ reader ใหม่มาต่อ loop เดิม; ถ้า resume attempt เองก็ fail (เช่น token หมดอายุ ได้ 410) **throw error เดิมจาก stream-read** ไม่ใช่ error ของการ resume เอง (fallback safety — คง semantics เดิมไว้ไม่ทำให้ error message งงกว่าเดิม); หมด attempt ก็ throw error ล่าสุดเหมือนกัน
  - Test coverage ใหม่: `download-resume.spec.js` (11 tests: create/get round-trip, session mismatch reject, lazy TTL expiry, MAX_ENTRIES eviction, discard idempotent), `uploads-downloads.spec.js` (+4 tests: fresh download มี `X-Resume-Token`, resume ด้วย `Range` header ได้ 206 + bytes ตรงกับไฟล์เต็มเป๊ะ (ใช้ raw `http.request()` + `req.destroy()` จำลอง connection ขาดจริง — supertest รอ response ครบเสมอ ทำแบบนี้ไม่ได้), unknown/expired token ได้ 410, cross-session resume ได้ 410), `ipc-transport.spec.js` (+5 tests: resume สำเร็จหลัง stream fail, ไม่มี resume token throw error เดิมทันที, cancellation priority เหนือ resume แม้มี token, resume attempt เอง fail แล้ว throw error เดิม, หมด `MAX_DOWNLOAD_RESUME_ATTEMPTS` แล้ว throw error ล่าสุด) — รวม bruno-server 372/372 (24/24 suites), bruno-app 1438/1438 (78/78 suites) ผ่านทั้งหมด lint clean
  - **บั๊ก test-design เจอระหว่างเขียน (ไม่ใช่บั๊ก product code)**: (1) test เดิมสมมติว่า resume token ยังใช้ได้หลัง download รอบแรกจบสมบูรณ์ผ่าน supertest — ผิด เพราะ token ถูก discard ทันทีที่ download จบสำเร็จ (ตามดีไซน์ที่ตั้งใจ) แก้โดยเปลี่ยนไปจำลอง connection ขาดจริงแทน; (2) ใช้ collectionName ยาว 300,000 ตัวอักษรเพื่อ pad ขนาดไฟล์ทดสอบ ดันไปโผล่ใน `Content-Disposition: filename=...` header ด้วย ทำให้ Node HTTP client reject ด้วย "Header overflow" (ดูเหมือน test ค้างเพราะ response ไม่เคย parse จบ ไม่มี event ไหน fire เลย) — แก้โดยแยก body size ออกจาก filename size (custom test handler เขียนไฟล์ใหญ่ลง disk โดยใช้ `collectionName` สั้นตามปกติ)
  - **Live-verified ด้วย bruno-server process จริง + curl/raw socket จริง (ไม่ใช่ mock)**: (1) fresh download ผ่าน `renderer:export-collection-zip` จริงบน collection ทดสอบ 3MB — response มี `X-Resume-Token`/`X-Content-SHA256` ครบ, `sha256sum` ตรงกับ header เป๊ะ; (2) resume token เดิมพอ download จบสมบูรณ์แล้ว retry ด้วย token นั้น → `410 Gone` ตามดีไซน์; (3) จำลอง connection ขาดจริงด้วย Node raw `http.request()` (`req.destroy()` หลังอ่านได้ไม่กี่ chunk แรก — ต้อง destroy เร็วมากตั้งแต่ chunk แรก ไม่งั้นไฟล์เล็กบน loopback flush ครบก่อนจะ destroy ทัน กลายเป็นนับว่า "จบสมบูรณ์" ไปซะก่อน) แล้ว resume ด้วย `Range: bytes=${partial}-` + token → ได้ `206 Partial Content`, ต่อ partial bytes กับ resumed bytes เข้าด้วยกันแล้ว `sha256sum`/`cmp` ตรงกับไฟล์ที่ download เต็มๆ เป๊ะทุก byte; (4) resume ด้วย token มั่ว/ไม่เคยมีอยู่จริง → `410 Gone`; (5) เปิด server พร้อม `BRUNO_SERVER_REQUIRE_AUTH=true` จริง สร้าง 2 session จริงผ่าน `POST /api/auth/session` (คนละ cookie/CSRF token) — session A สร้าง token จาก download ที่ขาดกลางทาง แล้วให้ session B ลอง resume ด้วย token ของ A → `410 Gone` (cross-session ปฏิเสธถูกต้อง), session A resume token ของตัวเอง → `206 Partial Content` สำเร็จ — ยืนยัน session-scoping ทำงานถูกต้องบนระบบที่รันจริงทุก layer ไม่ใช่แค่ unit test
- ยังไม่ทำ — **resume สำหรับ upload** (defer แยกออกจาก download-resume ข้างบนโดยตั้งใจ เพราะขอบเขตไม่สมมาตรกัน): download-resume ใช้ประโยชน์จาก HTTP `Range`/206 ที่เป็น standard อยู่แล้วได้เต็มที่ เพราะฝั่ง "อ่าน" มีไฟล์เต็มอยู่บน disk ให้ re-serve บางส่วนได้เลย — แต่ upload ไม่มี "ไฟล์เต็ม" ให้ resume จาก ต้องออกแบบ protocol ใหม่ทั้งชุด (chunked PUT/PATCH, server-side partial-file accumulation ระหว่างที่ยังอัพโหลดไม่ครบ, การจัดการ partial-file lifecycle/cleanup ที่ต่างจาก download-resume's "ไฟล์เดียวจบ" โดยสิ้นเชิง) — ขอบเขตใหญ่กว่า checksum และใหญ่กว่า download-resume มาก เทียบเท่า P1.2's idempotency-key scope ที่ defer ไว้แล้วเช่นกัน
...
- ✅ **idempotency key สำหรับ create/save ที่ retry ได้** — scope เป็น hand-picked allowlist 7 channel ที่เป็น pure create ระบุด้วยชื่อ/path ที่ handler เช็ค unique อยู่แล้ว (`renderer:new-request`, `renderer:new-folder`, `renderer:clone-folder`, `renderer:create-environment`, `renderer:create-global-environment`, `renderer:import-collection`, `renderer:import-collection-zip`) — ตาม note เดิมของ roadmap เองว่าเป็น per-endpoint product-scope decision ไม่ blanket-apply กับทุก channel (destructive channel ห้าม auto-retry แบบนี้เด็ดขาด)
  - Server (`security/idempotency.js` ใหม่): Map + lazy TTL expiry แบบเดียวกับ `auth.js`'s `sessions` pattern (`${channel}::${idempotencyKey}` → `{body, expiresAt}`, ไม่มี sweep timer เช็คตอนอ่านแล้วลบถ้าหมดอายุ), TTL default 5 นาที (`BRUNO_SERVER_IDEMPOTENCY_TTL_MS`), bounded ด้วย `MAX_ENTRIES` (default 1000, `BRUNO_SERVER_IDEMPOTENCY_MAX_ENTRIES`, evict entry เก่าสุดเมื่อเต็ม) — wiring เข้า `ipc-proxy.js`: เช็ค cache เร็วที่สุดเท่าที่ทำได้ (ก่อนทุก validation gate เลย เพราะ replay ไม่ต้องรัน validation/ownership check/handler ซ้ำ) hit แล้วตอบ response เดิมกลับทันที; เก็บเข้า cache เฉพาะ **response ที่สำเร็จเท่านั้น** หลัง dispatch เสร็จ ก่อน `res.json()` — ตั้งใจไม่ cache error response เพราะความล้มเหลวจริง (validation, disk error) อาจ transient หรือหมดอายุไปแล้วตอน retry ครั้งถัดไป การ pin ไว้จะทำให้ retry ที่ legit ทำไม่ได้เลยตลอด TTL window — ผลคือ scenario ที่ตั้งใจแก้ (response หายระหว่างทางแต่ write จริงสำเร็จแล้ว → retry เจอ false "already exists" error) แก้ได้เต็มที่ เพราะ retry ด้วย key เดิมถูก intercept ก่อนถึง handler เลย ไม่มีทางไปชน `fs.existsSync` throw
  - Client (`ipc-transport.js`): เพิ่ม method ใหม่ `retryableInvoke(channel, ...args)` (ไม่แตะ `invoke()` เดิมเลย — additive ล้วนๆ) เฉพาะ 7 channel ใน allowlist เดียวกัน (ก๊อปเป็น literal string ไว้ในไฟล์นี้เอง ไม่ pull `@usebruno/rpc-contract` เป็น dependency ใหม่ของ `bruno-app` แค่เพื่อ list สั้นๆนี้) generate idempotencyKey เดียวใช้ซ้ำทุก attempt แล้ว retry อัตโนมัติสูงสุด `RETRYABLE_INVOKE_MAX_ATTEMPTS` (3) ครั้ง แต่ retry เฉพาะ network-level failure เท่านั้น (`IpcTimeoutError` หรือ `fetch()`'s `TypeError` เช่น "Failed to fetch") — **ไม่ retry** error response จริงจาก server เลย (validation/business error ฯลฯ) เพราะอาจไม่ safe/มีประโยชน์ที่จะยิงซ้ำอัตโนมัติ; channel นอก allowlist fallback ไปเรียก `invoke()` ปกติตัวเดียวจบ (caller ไม่ต้องเช็ค eligibility เอง) — `ElectronTransport` ได้ `retryableInvoke()` เหมือนกันแต่แค่ delegate ไป `invoke()` เฉยๆ (IPC ใน Electron ไม่มี network hop ให้ retry)
  - ข้อจำกัดที่ยอมรับไว้ (documented ใน `idempotency.js`): ป้องกันเฉพาะ retry ที่ยิงมา *หลัง* attempt แรกจบที่ server แล้วเท่านั้น — 2 attempt ที่ genuinely in-flight พร้อมกันด้วย key เดียวกัน (เช่น client ยิง retry ก่อน handler ของ attempt แรกจะ return) ไม่ coordinate กันเลย ทั้งคู่รันได้จริง — full in-flight lock ตัดสินใจว่าเกินขอบเขตของรอบนี้
  - Test coverage ใหม่: `idempotency.spec.js` (10 tests: allowlist check, get/store, TTL expiry ด้วย mocked `Date.now`, `MAX_ENTRIES` eviction), `app-routes.spec.js` เพิ่ม describe block ใหม่ (4 integration tests ผ่าน real Express app + supertest: repeated key ไม่ re-invoke handler, different key invoke ใหม่, channel นอก allowlist ไม่ cache, handler error ไม่ถูก cache), `ipc-transport.spec.js` เพิ่ม describe block ใหม่ (4 tests: key เดียวกันทุก retry attempt, give up หลัง max attempts, ไม่ retry server error จริง, fallback เป็น plain invoke สำหรับ channel นอก allowlist) — full suite ผ่านทั้งคู่ (bruno-server 370/370 ขึ้นจาก 360, bruno-app 1437/1437 ขึ้นจาก 1433) lint clean
  - **Live-verified ด้วย server process จริง** (ไม่ mock): เปิด scratch collection จริงผ่าน `renderer:open-multiple-collections` + `renderer:add-collection-watcher` (real chokidar watcher) แล้วยิง `renderer:new-request` จริงผ่าน curl สร้างไฟล์ `.bru` จริงบน disk — (1) ลบไฟล์ทิ้งแล้ว retry ด้วย idempotencyKey เดิม → ได้ response เดิม `{"data":null}` กลับมาแต่ไฟล์**ไม่ถูกสร้างใหม่** (พิสูจน์ว่า handler ไม่ถูกเรียกซ้ำจริงๆ ไม่ใช่แค่ mock); (2) ยิง retry ด้วย key เดิมโดยไม่ลบไฟล์ (สถานการณ์เป้าหมายจริงตามที่ roadmap เขียนไว้) → ได้ replayed success กลับมาเหมือนกัน **ไม่ชน** error "already exists" ที่ real handler จะโยนถ้าไม่มี idempotency; (3) sanity check: ยิงด้วย key ใหม่ที่ path เดิม → ได้ error "already exists" จริงตามที่ควรจะเป็น (พิสูจน์ว่าไม่ได้ปิด duplicate-detection ทิ้งไปเฉยๆ)
  - ยังไม่ทำ: `renderer:add-cookie`, `renderer:create-workspace`, `renderer:create-workspace-dotenv-file`, `renderer:create-dotenv-file` (candidate ที่พบระหว่าง survey แต่ตัดสินใจไม่รวมรอบนี้ — ไม่ใช่ pure-create-by-unique-name/path แบบ 7 channel ที่เลือก ต้อง product-scope decision แยกต่อตัว); ยังไม่มี automatic retry บน UI action ใดๆ ในแอปจริง (`retryableInvoke()` เป็น infrastructure พร้อมใช้ แต่ยังไม่มี caller เรียกจริงใน production code — follow-up แยกต่างหาก pattern เดียวกับ P0.2/P1.5/opaque-handle ที่ backend เสร็จ UI ตามทีหลัง)
- ยังไม่ทำ — event sequence number และ resync หลัง reconnect — **สำรวจสถาปัตยกรรมแบบละเอียดแล้ว** (ดูรายละเอียดเต็มใน `find bug and Improvement.md`) พบว่าใหญ่กว่าที่ note เดิมบอกไว้อีก: (1) `EventBridge` (`ws/event-bridge.js`) ไม่มี buffer/log เลย เป็น pure fan-out ผ่าน `Set<ws>` เท่านั้น event ที่ยิงตอนไม่มีใคร subscribe หายถาวร; (2) วิธีที่ดูเหมือน "ฟรี" ที่สุด — เรียก `renderer:add-collection-watcher` ซ้ำ (ปิด-เปิด chokidar watcher ใหม่) — **ตรวจแล้วว่าใช้ไม่ได้จริง**: `collectionAddFileEvent` reducer เช็คชื่อไฟล์ก่อน แล้ว no-op เงียบๆถ้ามีอยู่แล้ว (แก้ไข content ระหว่าง disconnect จะไม่ถูก refresh) แถมไฟล์ที่ถูกลบระหว่าง disconnect ก็ไม่มีทาง detect เพราะ chokidar ไม่ re-emit `unlink` สำหรับไฟล์ที่ไม่มีอยู่แล้ว; (3) มีกลไกที่ถูกต้องอยู่แล้วจริงๆคือ `MountManager.remount()` + `buildTree()` (`services/mount/manager.js`, `tree-builder.js`) ที่ผลิต full tree ด้วย uid เสถียร (ใช้ `getRequestUid` cache เดียวกับ legacy watcher เป๊ะๆ) แล้วส่งผ่าน `main:collection-tree-loaded` ซึ่ง client's `mergeTreeItems` (ตรวจแล้วว่า generic พอ ไม่ผูกกับ MountManager) จะ diff/merge ให้ถูกต้องทั้ง add/update/remove — **แต่** mechanism นี้ทั้งหมดอยู่หลัง feature flag `state.app.preferences.cache.file.enabled` (default `false`) เป็น "v2 mount" opt-in เท่านั้น ไม่ใช่ default path ที่ผู้ใช้ทั่วไปเจอ; (4) `remount()` เองก็ยังไม่มี IPC channel ผูกไว้เรียกจาก renderer ได้เลยด้วยซ้ำ (dead code ที่ export ไว้เฉยๆ ใน `ipc/mount.js`); (5) การจะทำให้ resync ถูกต้องสำหรับ default (v1/legacy chokidar) path ต้อง extract logic parse ไฟล์ทีละไฟล์ออกจาก `collection-watcher.js`'s `add()` (ซึ่งมี branch เยอะมาก: format v1/v2, encryption, redaction, folder.bru meta) มาทำเป็น bulk parser แยกต่างหาก — ความเสี่ยงคือถ้า parse ผิด branch จะทำให้ collection data เพี้ยนตอน resync ซึ่งเสียหายกว่าการไม่มี feature นี้เลย — ตัดสินใจไม่รีบทำแบบเสี่ยงๆ ตาม **"ไม่ต้องเลื่อน ทำให้ครับไปเลย"** ที่หมายถึงไม่ผัดวันประกันพรุ่งเพราะขี้เกียจตัดสินใจ ไม่ใช่หมายถึงทำของที่เสี่ยง data corruption ทั้งที่รู้อยู่แล้วว่าเสี่ยง — ของจริงที่ทำได้ปลอดภัยตอนนี้คือเพิ่ม IPC channel ผูก `remount()` แล้ว trigger จาก client เฉพาะตอน user เปิด `file.enabled` (v2 mount) ไว้เท่านั้น ซึ่งกระทบผู้ใช้ส่วนน้อยมาก — ยังไม่คุ้มที่จะทำแยกเป็น increment ของตัวเองตอนนี้ รอ decision ว่าจะ invest ใน v1→v2 migration หรือ extract parser เป็นเรื่องใหญ่กว่านี้ค่อยกลับมาทำ
- ✅ **offline read-only cache สำหรับ UI state ล่าสุด** — self-scoped ตาม **"ไม่ต้องเลื่อน ทำให้ครับไปเลย"** (item นี้ถูก flag ไว้ก่อนหน้าว่า "ควรถามผู้ใช้ก่อน" เพราะเป็น product decision เรื่อง storage scope/invalidation policy — ตัดสินใจเองแทนการหยุดถาม): แทนที่จะ hydrate cached data เข้า `workspaces`/`collections` slice จริง (เสี่ยงพังเพราะ component หลายสิบตัวในแอปคาดหวัง field ที่ live collection object มีครบ เช่น `brunoConfig`, `environments`, `mountStatus` แต่ cached snapshot ไม่มี) เลือกออกแบบเป็น **read-only view ที่แยกเด็ดขาดจาก interactive data model**:
  - `utils/common/offline-cache.js` ใหม่ — ใช้ `idb` (`^7.0.0`, มีอยู่แล้ว) ตาม pattern เดียวกับ `utils/ai/chat-store.js` (module-level `dbPromise` singleton, guard ด้วย `typeof indexedDB === 'undefined'`) เก็บ snapshot เดียว (`id: 'latest'`) ใน object store `snapshot` ของ DB `bruno-offline-cache`. `buildOfflineSnapshot(state)` strip เหลือแค่ชื่อ/โครงสร้าง: workspace (`uid`/`name`/`pathname`/collection refs), collection tree (`uid`/`name`/`type`/`pathname`/`items` แบบ recursive เท่านั้น) และ global environment แค่ `uid`/`name` — **ไม่เก็บ request/response body, script, header, auth, draft, response history, และไม่เก็บ environment variable values เลย** ทั้งเพราะขนาดและเพราะ script/credential ที่ stale แล้วโชว์เป็นข้อมูล "ปัจจุบัน" ตอน offline จะทำให้เข้าใจผิด/เสี่ยง (verify แล้วด้วย real snapshot จาก live data จริงว่า serialize แล้วไม่มีคำว่า "secret"/"Bearer" หลงเหลือ)
  - `providers/ReduxStore/slices/offlineCache/index.js` ใหม่ — slice เล็กมาก เก็บแค่ `{ snapshot: null }`, reducer `setOfflineSnapshot`/`clearOfflineSnapshot`, ต่อเข้า root reducer (`providers/ReduxStore/index.js`) เป็น key `offlineCache` แยกจาก `workspaces`/`collections` โดยเจตนา
  - `providers/App/useOfflineCacheSync.js` ใหม่ (mount ใน `AppProvider`, Browser Bridge mode เท่านั้นผ่าน `!isElectronMode()`) — สอง responsibility: (1) **เขียน**: `store.subscribe` แบบ debounce 2 วินาที (pattern เดียวกับ `snapshotMiddleware`'s `DEBOUNCE_MS`) เขียน snapshot ใหม่ทุกครั้งที่ state เปลี่ยนแปลง แต่เขียนเฉพาะตอน connection เป็น `ONLINE` และมี live workspace อยู่แล้วเท่านั้น (ไม่เขียนทับ cache ดีๆด้วย state ว่างตอน loading); (2) **อ่าน**: หลัง grace period 4 วินาทีจาก boot ถ้า `workspaces.workspaces` ยังว่างอยู่ (ไม่มี live data มาถึงเลย) โหลด snapshot ล่าสุดจาก IndexedDB แล้ว dispatch เข้า `offlineCache` slice เท่านั้น (ไม่แตะ `workspaces`/`collections` จริง) — race-check ซ้ำหลัง async read เผื่อ live data มาถึงระหว่างอ่าน cache พอดี; เมื่อ connection กลับเป็น `ONLINE` จะ `clearOfflineSnapshot()` ทันที
  - `components/OfflineBanner/` ใหม่ — mount ที่ root ของ `AppProvider` (เหนือ `props.children`) แสดงเมื่อ (connection ไม่ใช่ `ONLINE`) และ (มี `offlineCache.snapshot`) เท่านั้น: ข้อความ "Offline — showing cached data from `<timestamp>`. Read-only until reconnected." พร้อม static, non-interactive list ของชื่อ collection ที่ cache ไว้ (ไม่มี click handler, ไม่ผูกกับ action ใดๆ — เป็นแค่ตัวหนังสือ ป้องกันไม่ให้ผู้ใช้เข้าใจผิดว่าคลิกแล้วจะเปิด request ได้)
  - Test coverage ใหม่ 4 ไฟล์ (16 tests รวม): `offline-cache.spec.js` (mock `idb`'s `openDB` เอง — ไม่มี pattern เดิมในโปรเจกต์ให้ใช้ต่อ ต้องสร้างใหม่ทั้งหมด — ครอบ build/strip logic, save/load round-trip, guard เมื่อไม่มี `indexedDB`), `offlineCache/index.spec.js` (reducer), `useOfflineCacheSync.spec.js` (fake timers ครอบ hydrate-after-grace-period, ไม่ hydrate ถ้า live data มาก่อน, debounced write เฉพาะตอน online, ไม่เขียนตอน offline, clear ตอนกลับ online), `OfflineBanner/index.spec.js` (ไม่ render ตอนไม่มี snapshot/ตอน online/ตอน Electron, render ข้อความ+ชื่อ collection ตอน offline มี snapshot) — full suite ผ่าน (bruno-app 1454/1454 ขึ้นจาก 1438) lint clean
  - Live-verify ด้วย real bruno-server + real Chromium ผ่าน Playwright จริง (build production bundle ด้วย `npm run build:web` แล้ว serve ผ่าน bruno-server's static-frontend path จริง ไม่ใช่ dev server): boot แอปจริงบน default workspace ของเครื่อง (มี collection จริงอยู่แล้ว), รอ debounced write แล้วอ่าน `bruno-offline-cache` DB ตรงจาก browser context ยืนยันว่ามี snapshot จริงที่ตรงกับ collection บนเครื่อง (`Test`, `bruno-testbench`) และไม่มีคำว่า secret/Bearer หลงเหลือ — **พบว่า kill process บน bruno-server ทั้งตัวใช้ verify ไม่ได้ตามที่วางแผนไว้แต่แรก** เพราะ deployment แบบ same-origin นี้ serve ทั้ง static shell และ API จาก process เดียวกัน kill แล้ว reload จะได้ `ERR_CONNECTION_REFUSED` ทันที (fetch index.html ไม่ได้เลย) ซึ่งไม่ใช่ scenario ที่ feature นี้ตั้งใจแก้ (feature นี้แก้ปัญหา "API/backend ล่มแต่ static shell ยังถึงได้" แบบ CDN+backend แยก deployment) — แก้โดยใช้ Playwright's `page.route()`/`page.routeWebSocket()` บล็อกเฉพาะ `/api/**` และ `/ws/**` จริงในระดับ network layer (ไม่แตะ server process เลย) แล้ว reload หน้า: banner ขึ้นจริงพร้อมข้อความ timestamp + ชื่อ collection ที่ถูกต้อง, ปลด block แล้ว reload อีกครั้งยืนยัน banner หายเมื่อ live data กลับมา — ครบทั้ง write-path, read/hydrate-path, และ recovery-path ด้วยกระบวนการจริงทั้งหมด
