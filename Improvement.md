...
### P1.1 Server File Explorer and Transfer Center 🟡 เสร็จบางส่วน (folder-picking modal + upload/download + progress/cancel + create-folder/rename + file-picker multi-select/preview + search/recent/favorites + Bridge-machine label + checksum verification จริงแล้ว; resume/opaque-handle ยังไม่ทำ)
